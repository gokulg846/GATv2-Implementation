...
class GATv2Conv(MessagePassing):
    ...
    def message(self, x_j, x_i, index, size_i):
      ...
      self.alpha = (x * self.attention).sum(dim=-1)
      self.alpha = softmax(self.alpha, index, num_nodes=size_i)
      self.alpha = F.dropout(self.alpha, p=self.dropout, training=self.training)
      return x_j * self.alpha.unsqueeze(-1)
...
def train(model, loader, optimizer, device):
  ...
  for data in loader:
      ...
      loss = F.mse_loss(out, data.y[:, 0].unsqueeze(1)) #Predicting the first property in QM-9 (Dipole Moment)
      loss.backward()
      optimizer.step()
...
def main():
    ...
    optimizer1 = torch.optim.Adam(model.parameters(), lr=0.001) #For GATv2
    optimizer2 = torch.optim.Adam(model2.parameters(), lr=0.001) #For GAT
    ...
    #plt.savefig('pred_vs_true_gatv2.png')
    #plt.close()
    
    #plot_true_vs_pred(model2, test_loader, device)
    #plt.savefig('pred_vs_true_gat.png')
    #plt.close()
    
    # Visualize attention scores for both models
    #visualize_attentions(model, model2, test_loader, device)
